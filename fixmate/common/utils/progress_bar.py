...
from fixmate.logger import console, logger
...
class PatchflowProgressBar:
    ...
    def __init__(self, fixflow: Step):
        self.__step_counter = Counter()
        self.__current_progress = 0.00
        self.__fixflow_name = fixflow.__class__.__name__

        fixflow_run_func = fixflow.run

        def inner_run():
            try:
                return fixflow_run_func()
            finally:
                self.__progress_bar_update(
                    description=f"[bold green]Finished {self.__fixflow_name}", completed=self.__MAX_PROGRESS
                )

        fixflow.run = inner_run
    ...
    def __progress_bar_update(self):
        ...
        task_id = progress.add_task(
            description=f"[bold green]Running {self.__fixflow_name}",
