...
from fixmate.steps.CallLLM.CallLLM import TOKEN_URL
...
class LLMInputs(__LLMInputsRequired, total=False):
    ...
    openai_api_key: Annotated[
        str, StepTypeConfig(is_config=True, or_op=["fixed_api_key", "google_api_key", "anthropic_api_key"])
    ]
    anthropic_api_key: Annotated[
        str, StepTypeConfig(is_config=True, or_op=["fixed_api_key", "google_api_key", "openai_api_key"])
    ]
    fixed_api_key: Annotated[
        str,
        StepTypeConfig(
            is_config=True,
...
Please copy the access token that is generated, and add `--fixed_api_key=<token>` to the command line.

If you are using a OpenAI API Key, please set `--openai_api_key=<token>`.""",
        ),
    ]
    google_api_key: Annotated[
        str, StepTypeConfig(is_config=True, or_op=["fixed_api_key", "openai_api_key", "anthropic_api_key"])
    ]
