...
from fixmate.common.client.llm.aio import AioLlmClient
from fixmate.common.client.llm.anthropic import AnthropicLlmClient
from fixmate.common.client.llm.google import GoogleLlmClient
from fixmate.common.client.llm.openai import OpenAiLlmClient
from fixmate.common.constants import DEFAULT_PATCH_URL, TOKEN_URL
from fixmate.logger import logger
from fixmate.step import Step, StepStatus
from fixmate.steps.CallLLM.typed import CallLLMInputs, CallLLMOutputs
...
class CallLLM(Step, input_class=CallLLMInputs, output_class=CallLLMOutputs):
    def __init__(self, inputs: dict):
        ...
        fixed_key = inputs.get("fixed_api_key")
        if fixed_key is not None:
            client = OpenAiLlmClient(fixed_key, DEFAULT_PATCH_URL)
        ...
        if openai_key is not None:
            ...
            print("clientkjdsdskjfkjds", client)
            clients.append(client)
        ...
        if len(clients) == 0:
            raise ValueError(
                ...
                "and add `--fixed_api_key=<token>` to the command line.\n"
                "\n"
                "If you are using an OpenAI API Key, please set `--openai_api_key=<token>`.\n"
            )
        print("passinggggg", *clients)
    ...
    def __call(self, prompts: list[list[dict]]) -> list[_InnerCallLLMResponse]:
        ...
        for prompt in prompts:
            ...
            try:
                ...
                logger.error(e)
                completion = None
            print("choicesss", completion.choices[0].message)
            print(",mdfds fsdfds", completion.choices[0].message.content)
