...
from fixmate.common.utils.user_config import get_user_config
from fixmate.common.utils.utils import get_current_branch, is_container
...
class PatchedClient(click.ParamType):
    TOKEN_URL = "https://app.fixed.codes/signin"
    DEFAULT_PATCH_URL = "https://fixmate.fixed.codes"
    ...
    def _post(self, **kwargs) -> Response | None:
        try:
            ...
            logger.error(f"Unable to establish connection to fixed server: {e}")
            return None
    ...
    async def _public_telemetry(self, fixflow: str, inputs: dict[str, Any]):
        ...
        requests.post(
            ...
            json=dict(
                ...
                environment=dict(
                    ...
                    cli_version=metadata.version("fixmate-cli"),
                    is_container=is_container(),
                ),
            ),
        )

    def send_public_telemetry(self, fixflow: str, inputs: dict):
        try:
            _thread = Thread(target=asyncio.run, args=(self._public_telemetry(fixflow, inputs),))
            _thread.start()
        except Exception as e:
            logger.debug(f"Failed to send public telemetry: {e}")

    @contextlib.contextmanager
    def fixed_telemetry(self, fixflow: str, inputs: dict):
        ...
        try:
            ...
            fixflow_run_id = self.record_fixflow_run(fixflow, repo, self.__handle_telemetry_inputs(inputs))
        except Exception as e:
            logger.error(f"Failed to record fixflow run: {e}")
            yield
            return

        if fixflow_run_id is None:
            yield
            return
        ...
        try:
            ...
            try:
                self.finish_record_fixflow_run(fixflow_run_id, fixflow, repo)
            except Exception as e:
                logger.error(f"Failed to finish fixflow run: {e}")

    def record_fixflow_run(self, fixflow: str, repo: Repo, inputs: dict) -> int | None:
        head = get_current_branch(repo)
        branch = head.remote_head if head.is_remote() else head.name

        response = self._post(
            url=self.url + "/v1/fixmate/",
            headers={"Authorization": f"Bearer {self.access_token}"},
            json={"url": repo.remotes.origin.url, "fixflow": fixflow, "branch": branch, "inputs": inputs},
        )
        ...
        if not response.ok:
            logger.error(f"Failed to record fixflow run with status code {response.status_code}, msg:{response.text}")
            return None

        logger.debug(f"Patchflow run recorded for {fixflow}")
        return response.json()["id"]

    def finish_record_fixflow_run(self, id: int, fixflow: str, repo: Repo) -> None:
        response = self._post(
            url=self.url + "/v1/fixmate/",
            headers={"Authorization": f"Bearer {self.access_token}"},
            json={
        ...
        if not response.ok:
            logger.error(f"Failed to finish fixflow run with status code {response.status_code}, msg:{response.text}")
            return
